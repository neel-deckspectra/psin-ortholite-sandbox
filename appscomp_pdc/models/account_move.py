...
class AccountMove(models.Model):
    ...
    draft_payment_count = fields.Integer(string='PDC-Payment', copy=False,
                                         compute='_compute_draft_payment_count')
    payment_ids = fields.Many2many('account.payment', compute='_compute_customer_invoice_payment',
                                   string='Payment(s)', store=True)
    payment_ids_count = fields.Integer(string='Payment Count')

    def _compute_customer_invoice_payment(self):
        for item in self:
            domain = [('reconciled_invoice_ids.id', '=', item.id)]
            payment_ids = self.env['account.payment'].sudo().search(domain)
            item.payment_ids = payment_ids
            item.payment_ids_count = len(payment_ids)
    ...
    def _compute_draft_payment_count(self):
        self.draft_payment_count = self.env['account.payment'].sudo(). \
            search_count([('ref', '=', self.name), ('partner_id', '=', self.partner_id.id)])

    def get_invoice_pdc_draft_payment(self):
        ...
        return {
            'name': _('Payment Reference'),
            'res_model': 'account.payment',
            'type': 'ir.actions.act_window',
            'view_mode': 'tree,form',
            'views': [(tree_view.id, 'tree'), (form_view.id, 'form')],
            'domain': [('ref', '=', self.name), ('partner_id', '=', self.partner_id.id)],
        }
